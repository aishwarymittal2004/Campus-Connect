...
import { Train, Clock, ArrowLeft, ArrowRight, IndianRupee } from "lucide-react";
...
export function TrainSchedulesPage() {
  ...
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      try {
        const response = await apiClient.get(`/routes/train-schedules?source=${encodeURIComponent(source)}&dest=${encodeURIComponent(dest)}`);
  ...
  return (
    <div className="mx-auto max-w-4xl pb-12">
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold tracking-tight">Train Schedules</h1>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-lg font-medium">No train schedules found</p>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex justify-between items-center relative">
                          <div className="w-1/3">
                            ...
                            <p className="text-sm font-medium">{leg.departure_station}</p>
                          </div>
                          ...
                          <div className="w-1/3 text-right">
                            ...
                            <p className="text-sm font-medium">{leg.arrival_station}</p>
                          </div>
                        </div>
                      </div>
                      
                      <div className="md:w-48 bg-muted/20 rounded-lg p-3 flex flex-col justify-center">
                        <p className="text-xs text-muted-foreground mb-2">Available Classes:</p>
                        <div className="flex flex-wrap gap-1.5">
                          {leg.classes.map((cls: string) => (
